...
When `.aigame/automation.json` has `mode: ai_staging`, make all concept-direction and product decisions, prefer the pitch marked recommended unless evidence makes another pitch materially safer, accept the CLI-generated agent approvals, and continue without asking for creative permission. Auto-approve ordinary project-local edits, allowlisted commands, tests, branches, commits, and PR preparation. Do not fabricate an approval file; the CLI records the bound agent approval. At `commit_blueprint_inputs`, commit the canonical concept records, run `aigame concept next --json` to obtain the refreshed commit-bound request, then run `aigame concept finalize --apply --json` without a human approval file.
